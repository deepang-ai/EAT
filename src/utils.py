...
def patchify(imgs: torch.Tensor, path_size: int):
    """
    Convert input image to patches, image shape is a cube
    imgs: (N, modality, H, W, D)
    x: (N, L, patch_size_hw**2 * patch_size_depth) [batch_size, num_patches, size of each patch]
    """
...
def unpatchify(x: torch.Tensor, image_size: int, path_size: int, modality: int):
    """
    Map back from image patches to original image
    x: (N, num_patches, size of each patch)
    imgs: (N, C, H, W, D)
    """
...
def random_masking(x, mask_ratio):
    ...
    ids_shuffle = torch.argsort(noise, dim=1)  # ascend: small is keep, large is remove; dim=1 sort by seq_length dimension
    ids_restore = torch.argsort(ids_shuffle, dim=1)  # Restore sequence via ids_restore

    # keep the first subset
    ids_keep = ids_shuffle[:, :len_keep]  # patches to keep
    # Gathers values along an axis specified by dim. https://zhuanlan.zhihu.com/p/352877584
    x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))  # Token sequence kept after masking

    # generate the binary mask for decoder: 0 is keep, 1 is remove
    mask = torch.ones([N, L], device=x.device)
    mask[:, :len_keep] = 0  # nomask value is 0, mask value is 1
    # unshuffle to get the binary mask
    mask = torch.gather(mask, dim=1, index=ids_restore)  # Mask order of token sequence from original image
